...
| `uid` / `gid`                                        | `null`                            | Shorthand: adds `mover.securityContext.{runAsUser,runAsGroup}` + `mover.podSecurityContext.fsGroup` to **both** the `SnapshotPolicy` and `Restore` mover specs. Each defaults to the other when only one is set (the common single-user-app case). Matters most under `backup.copyMethod: Direct` (the source is read-only mounted, so `fsGroup` alone can't make it readable). Root (`0`) isn't supported by this shortcut — rejected at the schema level; set `backup.mover`/`populate.mover` directly for that. An explicit `backup.mover`/`populate.mover` field still wins, field-by-field.                                     |
...
| `persistence.<name>.existingClaim`                   | `false`                           | This entry's key is the name of a PVC that already exists and isn't created/managed by this chart. No `PersistentVolumeClaim` is rendered; `Restore`/`SnapshotPolicy`/`SnapshotSchedule` still are. `Restore` stays `target.populator` (not `target.pvcRef`) — inert on this PVC by design, since it's presumably already `Bound`; see "First deploy vs. steady state" above.                                                                        |
...
| `persistence.<name>.uid` / `.gid`                    | `null`                            | Per-entry override of the top-level `uid`/`gid` shorthand — same effect, scoped to this entry only. Handy when one release's several entries belong to apps with different owners.                                                                                                                                                                                                                                                                    |
