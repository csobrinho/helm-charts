...
```bash
helm install esphome-config oci://ghcr.io/csobrinho/helm-charts/kopiur-pvc \
  --namespace esphome \
  --set pvc.size=10Gi \
...
Or from a Kustomize `helmCharts:` entry (single pvc):

```yaml
helmCharts:
  - name: kopiur-pvc
    repo: oci://ghcr.io/csobrinho/helm-charts
    releaseName: esphome-config
    namespace: esphome
    version: 0.1.0
    valuesInline:
      pvc:
...
| ------------------------------------------------- | --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `nameOverride`                                    | `""`                              | Name of the single PVC when `pvcs` is empty (defaults to the release name). Ignored once `pvcs` is set.                                                                                                                                                                                                                                                                                                                                                 |
...
| `pvcs`                                            | `{}`                              | Define multiple PVCs in one release, keyed by PVC name; each entry sparse-overrides the top-level `pvc`/`populate`/`backup` blocks. Leave empty for a single PVC named `nameOverride`. See the "Multiple pvcs" example above.                                                                                                                                                                                                                          |
...
`<name>-values.yaml` files as new cases come up (e.g. a `pvcs`-map example).
